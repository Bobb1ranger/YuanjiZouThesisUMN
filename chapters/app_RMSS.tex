...
\begin{table}[h]
    ...
    \hline
        $m_c$ & 2 kg& Mass of cart\\
        $m_p$ & 0.1 kg& Mass of pendulum\\
        $l$ & 1 m &Length of pendulum\\
        $f_s$ & \(60\mathrm{Hz}\) & Sampling Frequency\\
        \hline
    \end{tabular}
\end{table}
Let $x=[\theta,\dot{\theta},s, \dot{s}]^T$, the upper settings yield following discrete time state space matrices for plant $P$:
\begin{equation}
    \begin{split}
    &   A = \left[\begin{array}{cccc}
            1.0014 & 0.0167 & 0 & 0\\
            0.1716 & 1.0014 & 0 & 0\\
           -0.0001 & -0.0000 &  1 & 0.0167 \\
           -0.0082 & -0.0001 &       0 & 1
...
$$
When there is $e$ probability of packet drop in each channel, see Figure~\ref{fig:CLblk}.d. We can accordingly adjust the optimal controller to generate the same closed loop mean system as below,
...
\end{align}
\end{subequations}
$[\hat{K},\hat{L}]^T$ is a stabilizing controller for the system $[P,-z^{-1} I_{2}]$. So we can use $H_\infty$ synthesis to obtain this controller. The solution of $H_\infty$ synthesis is guaranteed to be proper and well-posed. From equation (\ref{eqn:transfrom_LK}), we know $L$ and $K$ are proper and well-posed for any arbitrary value $e$ takes. 
