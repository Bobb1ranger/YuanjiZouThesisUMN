...
\section{State-of-the-art robust analysis and synthesis}
...
\cite{Meansquarenorm} has first proposed a robust control framework for MIMO LTI systems subject to memoryless random gains. It derives a necessary and sufficient condition for MS stability, showing that MS stability can be equivalently thought of as robust mean stability. The framework has been extended to various network control settings \cite{padmasola2006tradeoffs, Padma06ACC, Jeff11, MattACC15, Matt2017thesis} which are relevant in this paper. In particular, \cite{Ma2015} is the first to show that MS performance is equivalent to an augmented MSS problem in the same vain of the analogous robust performance problems for deterministic uncertainty. 

Meanwhile, A control system need to tolerate and even actively compensate intermittent communications beyond those accounted for in the nominal control design. A common practice is to monitor the communication channel status and impute missing signals accordingly. \cite{Matt2014cvxMScontrol, Matt2014cvxMScontrol_act, Matt2017thesis} demonstrate that this compensation mechanism can be formulated as a linear component and synthesized via convex optimization for a large class of systems. 
...
\cite{Seiler2003BRLjs,MixH_2infcontrolforrandomsys} focused on the $\mathcal{H}_\infty$ guarantees of random systems. \cite{CtrlPackDrop_uncertain} considered the norm-bounded model uncertainty in the random packet drop problem and found a sufficient condition for different controller schemes to ensure stability.
...
\begin{figure}[h]
    ...
    \label{fig:sysconstr}
\end{figure}
Determine if the following dynamical system \(M\), with block diagram shown in Figure~\ref{fig:sysconstr},
...
\end{align}
\end{subequations}
interconnecting with $\forall\; \Delta(\colon q \mapsto p) \in \mathcal{B}_{\bm{\Delta}}^{\mathrm{LTI}}$ and \(\forall\; \bar{\Delta}(\colon y \mapsto v) \in \mathcal{B}_{\bar{\Delta}}^{\mathrm{stoc}}\) is mean square. 
...
In this chapter, we recapture $\mu$-analysis in robust control. We mainly follow the classical definition and notations from~\cite{packard_mainloop}.
...
\subsection{Mean Square(MS) stability and MS norm}
...
\end{equation}
\cite{Meansquarenorm} gives out the definition, based on the state-space realization in \ref{eqn:DT_statespace}, for the MS closed-loop stability. Interconnect the output $y$ with input $v$, $v=\bar{\Delta}y$, with the stochastic uncertainty $\bar{\Delta}$ satisfies the following assumptions:
...
$G(z)=C(zI-A)^{-1}B+D$, a $n_y\times n_v$ (We require that $n_y = n_v$) transfer function matrix. Define the mean square norm:
...
In this section, we generalize the preliminary result from~\cite{Meansquarenorm} to the case where the plant model is subject to LTI-structured uncertainty. We next present formulations and solutions to new robust mean-square performance and stability problems.
...
For each class of $\mathcal{B}_{\bm{\Delta}}$, there is a class of corresponding multipliers which commute with $\Delta$:
$$
...
Given a nominal plant $M$ defined as \ref{Discrete_statespace_Mix}. If $G_\Delta$ is well defined and robustly stable for $\Delta\in\mathcal{B}_{\bm{\Delta}}^{\mathrm{LTI}}$,  then the system $M$ is said to have robust mean square norm $\|G\|_{MS,r}$ if 
\begin{equation*}
    \|G\|_{MS,r} \doteq \sup_{\Delta \in \mathcal{B}_{\bm{\Delta}}^{\mathrm{LTI}}}\max_{i \in \{1,\hdots,n_y\}}\sqrt{\sum_{j=1}^{n_v}{\|G_{\Delta,ij}\|_2^2}} < \infty
\end{equation*}
\end{defn}
...
\begin{equation*}
    \|G\|_{MS,r}^2=\max_{i=1,\hdots,p}\sup_{\Delta \in \mathcal{B}_{\bm{\Delta}}^{\mathrm{LTI}}}\sum_{j=1}^p{\|G_{\Delta,ij}\|_2^2}
\end{equation*}
...
Given a nominal system $M$ defined as \ref{Discrete_statespace_Mix}. Assume $G_\Delta= \mathcal{F}_u(M, \Delta)$ is well defined and robustly stable for $\Delta\in\mathcal{B}_\Delta^{\mathrm{LTI}}$. Then the following condition holds.
\begin{equation*}
    \mu_{\bm{\Delta},\bar{\Delta}} (M) = \rho_{r}(\tilde{G})\doteq \sup_{\Delta \in \mathcal{B}_{\bm{\Delta}}^{\mathrm{LTI}}}\rho(\tilde{G}_{\Delta}) < 1
\end{equation*}    
\end{cor}
...
Towards the other end, Theorem~\ref{thm:robMSnorm} allows us to simply compute for a fixed $\theta$
$$
\sup_{\Delta\in\mathcal{B}_\Delta^{\mathrm{LTI}}}\|\theta G_\Delta\theta^{-1}\|^2_{MS}
$$
By minimizing over weight $\theta>0$, we will obtain
$$
\bar{\rho}_r(\tilde{G}) \doteq \inf_{\theta>0}\sup_{\Delta\in\mathcal{B}_\Delta^{\mathrm{LTI}}}\|\theta G_\Delta\theta^{-1}\|^2_{MS}
$$

Note that  
\begin{equation*}
\rho_r(\tilde{G})=\sup_{\Delta\in\mathcal{B}_\Delta^{\mathrm{LTI}}}\inf_{\theta>0}\|\theta G_\Delta\theta^{-1}\|^2_{MS}\leq \inf_{\theta>0}\sup_{\Delta\in\mathcal{B}_\Delta^{\mathrm{LTI}}}\|\theta G_\Delta\theta^{-1}\|^2_{MS} = \bar{\rho}_r(\tilde{G}).
\end{equation*}
So what we are computing is, in general, an upper bound to $\rho_r(\tilde{G})$. We leave to future research this investigation into reducing conservatism. 
...
Mathematically, if $n_v = 1$, $\rho(\tilde{G})_r = \bar{\rho}(\tilde{G})_r$.
\end{rem}


We have the main result of this section. The following theorem establishes a convex feasibility problem for verifying robust MS stability. 
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.5]{figures/RMSSfigure/Plant1.png}
...
We leave the optimal controller design in the presence of both stochastic packet drop and model uncertainty to future research. Herein, we evaluate the limitations of an existing controller. Such a controller has the structure shown in figure (\ref{fig:CLblk}.a), which is motivated by \cite{Jeff2005Packet}, to effectively deal with dropouts, but otherwise is an $\mathcal{H}_\infty$ controller based on the mean system shown in figure (\ref{fig:CLblk}.c). Need more details on how the $\mathcal{H}_\infty$ controller is designed, see appendix. 

\begin{figure}[htbp]
    \centering
    \includegraphics[width = 0.75\linewidth]{figures/RMSSfigure/ClosedloopBDG.PNG}
...
In figure (\ref{fig:CLblk}.c), the mean system has an equivalent loop transfer function of $\hat{K}(I+z^{-1}\hat{L})^{-1}P$. From the controller we've designed, the loop transfer function has properties shown in Table~\ref{tab:margin}. The margins are sufficient. We can conclude that this controller is robust against model uncertainty. 
\begin{table}[h]
    ...
    \hline
        Margin  & Value \\
    \hline
        Gain margin & $[0.5247,1.9055]$\\
        Phase margin & $[-34.62,34.62]\deg$\\
        Disk margin & $ 0.6233$\\
        $\mathcal{H}_\infty$ norm & 1.4627 \\
        \hline
    \end{tabular}
\end{table}
...
$$
Figure~\ref{fig:eta_e} plots the largest allowed probability of packet drop against the uncertainty \(\ell_2\) induced norm bound $\eta$ in controller gain. 
\begin{figure}[h]
    \centering
